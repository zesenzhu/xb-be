...
export class NotificationService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    this.logger.log('初始化系统通知与监控服务，启动 2 分钟服务器健康轮询任务...');
  ...
  async createNotification(data: {
    ...
    try {
      ...
      this.logger.log(`[系统通知中心] 生成新通知 [${data.level}] ${data.title}`);
      return notification;
    } catch (err) {
   ...
   * 获取所有通知列表 (支持分页和未读过滤)
   */
  async findAll(query: { isRead?: boolean; level?: string; page?: number; limit?: number }) {
  ...
  private async checkServerHealth() {
    ...
    if (memoryUsageRate > 90) {
      if (now - this.lastMemoryAlertTime > 3600000) { // 1 小时冷喷
    ...
    try {
      ...
      if (usePercentPart) {
        const diskUsageRate = parseInt(usePercentPart.replace('%', ''), 10) || 0;
        if (diskUsageRate > 90) {
          if (now - this.lastDiskAlertTime > 7200000) { // 2 小时冷喷
    ...
    try {
      ...
      if (logTotalCount > 1000000) { // 超过 100 万条
        if (now - this.lastLogVolumeAlertTime > 86400000) { // 24 小时冷喷
